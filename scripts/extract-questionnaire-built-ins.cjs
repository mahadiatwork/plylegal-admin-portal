...
function describeField(key, rawSchema, context, prefix, depth = 0) {
  ...
  const descriptor = { id: `${prefix}-${key}`.replace(/[^A-Za-z0-9_.:-]/g, '-').slice(0, 128), answerKey: key, label: ui.label, type: 'text', required: false };
  ...
  if ((array || object) && depth < 4) {
    ...
    let fields = schemaProperties(rowObject, rowResolved.context).map(prop => describeField(propertyName(prop.key), prop.value, rowResolved.context, `${prefix}-${key}`, depth + 1));
  ...
  if (ui.element && descriptor.type === 'text') {
    if (nodes(ui.element, node => node.type === 'JSXElement' && jsxName(node) === 'Textarea').length) descriptor.type = 'textarea';
    const select = nodes(ui.element, node => node.type === 'JSXElement' && jsxName(node) === 'Select')[0];
    if (select) {
      const options = nodes(select.node, node => node.type === 'CallExpression' && node.callee?.property?.name === 'map').map(({ node }) => staticOptions(node.callee.object, context)).find(Boolean);
      const direct = nodes(select.node, node => node.type === 'JSXElement' && jsxName(node) === 'SelectItem').map(({ node }) => ({ value: literal(attribute(node, 'value')), label: cleanLabel(textOf(node)) })).filter(option => typeof option.value === 'string' && option.label);
      if (options?.length || direct.length) { descriptor.type = 'select'; descriptor.options = options || direct; }
    }
  }
  if (ui.element) {
...
function mainSchema(context) {
  ...
  if (resolver) return resolveIdentifier(resolver.arguments[0]?.type === 'ConditionalExpression' ? resolver.arguments[0].alternate : resolver.arguments[0], context);
  ...
  if (defaults) return { node: inferDefault(defaults), context };
  for (const [name, node] of context.variables) if (/^(formSchema|contactFormSchema|custodySchema)$/.test(name) && schemaCall(node, 'object')) return { node, context };
  return null;
}
function extractPage(route, title, sectionKey, profileRole) {
  ...
  let questions = schemaProperties(object, schema?.context || context).map(prop => describeField(propertyName(prop.key), prop.value, context, id));
  ...
  if (!questions.length && route.endsWith('/all-applicants/languages')) {
    ...
    fields.forEach(field => { if (['speak', 'read', 'write'].includes(field.answerKey)) field.type = 'checkbox'; });
